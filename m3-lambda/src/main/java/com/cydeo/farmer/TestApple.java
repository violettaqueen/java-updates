...
public class TestApple {
    ...
    public static void main(String[] args) {
        ...
        List<Apple> heavyApple = filterApples(inventory, new AppleHeavyPredicate()); // pass one object ApplePredicate type
    ...
    private static List<Apple> filterApples(List<Apple> inventory, ApplePredicate applePredicate) { // interface
        ...
        for (Apple apple : inventory) {
            if (applePredicate.test(apple)) { //test apple and if it's true
                result.add(apple);
            }
        }
        return result;
